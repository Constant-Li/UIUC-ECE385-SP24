//-------------------------------------------------------------------------
//    Color_Mapper.sv                                                    --
//    For use with ECE 385 text_mapper                                   --
//    University of Illinois ECE Department                              --
//-------------------------------------------------------------------------


module  color_mapper ( input  logic [9:0]  DrawX, DrawY,                     
                       input logic [3:0] s10,s1,m1,m10,
                       input  logic [31:0] mem_reg[70],
                       input logic [23:0] color_reg[24],
                       output logic [7:0]  Red, Green, Blue );
    
    // col and row are offset into registers
    logic [9:0] col, row;
    logic [9:0] text_col, text_row;
    // cur_reg stores current register in VRAM
    logic [31:0] cur_reg; 
    logic [31:0] text_reg;
    logic [31:0] extra_reg;
    
    logic [7:0] s10_index;
    logic [7:0] s1_index;
    logic [7:0] m1_index;
    logic [7:0] m10_index;
    
    // stores current glyph to draw
    logic [7:0] cur_blk;
    logic [7:0] text_blk;
    
    // stores the character we are drawing, use it as index into font
    logic [4:0] addr;
    logic [10:0] text_addr;
    
    logic [23:0] data;
    logic [7:0] text_data;
    
    logic [4:0] read_data;
    
    // get the pixel inside
    logic [4:0] inside_x, inside_y;
    logic [4:0] text_inside_x, text_inside_y;

    
    // get the pixel inside
    logic [9:0] inside_x_wasd, inside_y_wasd;
    logic [15:0] read_addr;
    
    font_rom font(
        .addr(addr),
        .data(data)
    );
    
    text_font text(
        .addr(text_addr),
        .data(text_data)
    );
    
    ram2 header2(
		.read_address(read_addr),
        .data_Out(read_data)
    );
    
    // transfer numbers from 0-9 => font offset ex. 0x30, 0x31
    number sec10(
        .number(s10),
        .index(s10_index)
    );
    number sec1(
        .number(s1),
        .index(s1_index)
    );
    number min1(
        .number(m1),
        .index(m1_index)
    );
    number min10(
        .number(m10),
        .index(m10_index)
    );
    
    
   logic [18:0] read_addr_wins;
   logic [2:0] read_data_wins;
   logic [2:0] wins;
   logic [6:0] index_x_wins, index_y_wins;
   
   ramWINS winsram(
        .read_address(read_addr_wins),
        .wins(wins),
        .data_Out(read_data_wins)
    );
    
    
   logic [18:0] read_addr_press;
   logic [2:0] read_data_press;

   logic [6:0] index_x_press, index_y_press;  
   
   press_ram presssram(
        .read_address(read_addr_press),
        .data_Out(read_data_press)
    );
    
   // checking the winner
   always_comb
   begin
        if (mem_reg[65] == 1) 
        begin
            if (mem_reg[66] == 1) // BLACK
            begin
                wins = 3'b001;
            end
            else if (mem_reg[66] == 2) // WHITE
            begin
                wins = 3'b010;
            end
        end
        else if (mem_reg[65] == 2)
        begin
            if (mem_reg[66] == 1) // HUMAN
            begin
                wins = 3'b011;
            end
            else if (mem_reg[66] == 2) // AI
            begin
                wins = 3'b100;
            end
        end
        else
        begin
            wins = 3'b000;
        end
        
   end
   
    // draw boarder
    always_comb
    begin
        // if it's inside the board
        if ((DrawX >= 10'd127 && DrawX <= 10'd511) && (DrawY >= 10'd47 && DrawY <= 10'd431))
            begin
            // col = DrawX / 96
            col = (DrawX - 10'd127) / 96; 
            // row = DrawY / 24
            row = (DrawY - 10'd47)  / 24; 

            // DrawX % 24
            inside_x = (DrawX - 10'd127) % 24;
            inside_y = (DrawY - 10'd47) % 24;
            
            addr = inside_y;
            // current 16*16 glyph to draw
            cur_reg = mem_reg[col + row * 4];
            // inside_x % 4 to determine one of four glyph to draw

            case(((DrawX - 10'd127) % 96) / 24)
                2'b00:
                    cur_blk = cur_reg[7:0];
                2'b01: 
                    cur_blk = cur_reg[15:8];
                2'b10:
                    cur_blk = cur_reg[23:16];
                2'b11: 
                    cur_blk = cur_reg[31:24];
            endcase             
            
            // if we are drawing the boarder of 16*16 glyph
            if(inside_x == 5'd0 || inside_x == 5'd23 || inside_y == 5'd0 || inside_y == 5'd23)
                begin
                    // drawing highlighting boarder(red)
                    if(cur_blk >= 4'h3)
                        begin
                            Red   = 8'hff;
                            Green = 8'h00;
                            Blue  = 8'h00;
                        end
                    // drawing normal boarder(black)
                    else
                        begin
                            Red   = 8'h00;
                            Green = 8'h00;
                            Blue  = 8'h00;
                        end
                end
             
             else
                begin
                    // empty glyph, just the background(brown)
                    if(cur_blk == 4'h0 || cur_blk == 4'h3)
                        begin
                            Red   = 8'hf0;
                            Green = 8'ha0;
                            Blue  = 8'h00;
                        end
                    // black chess, with brown background
                    else if(cur_blk == 4'h1 || cur_blk == 4'h4)
                        begin
                            // if we are drawing the chess
                            if(data[inside_x])
                                begin
                                    Red   = 8'h00;
                                    Green = 8'h00;
                                    Blue  = 8'h00;
                                end
                            // we are drawing the background(brown)
                            else
                                begin
                                    Red   = 8'hf0;
                                    Green = 8'ha0;
                                    Blue  = 8'h00;
                                end
                        
                        end
                    // white chess, with brown background
                    else 
                        begin
                            // if we are drawing the chess
                            if(data[inside_x])
                                begin
                                    Red   = 8'hff;
                                    Green = 8'hff;
                                    Blue  = 8'hff;
                                end
                            // we are drawing the background(brown)
                            else
                                begin
                                    Red   = 8'hf0;
                                    Green = 8'ha0;
                                    Blue  = 8'h00;
                                end      
                        end        
                end
            end
        // draw text "GOMOKU"
        else if ((DrawX >= 10'd272 && DrawX <= 10'd399) && (DrawY >= 10'd10 && DrawY <= 10'd41))
            begin
                text_inside_x = ((DrawX - 10'd272) >> 1) & 5'b00111;
                text_inside_y = ((DrawY - 10'd10) >> 1) & 5'b01111;
                
                // text_reg = "GOMO"
                text_reg = 32'h4f4d4f47;
                // extra_reg = "KU\\"
                extra_reg = 32'h0000554b;
                case((DrawX - 10'd272) >> 6)  // DrawX / 64
                    2'b00:
                        case((DrawX - 10'd272) >> 4)  // DrawX / 16
                            2'b00:
                                text_blk = text_reg[7:0];
                             2'b01: 
                                text_blk = text_reg[15:8];
                            2'b10:
                                text_blk = text_reg[23:16];
                            2'b11: 
                                text_blk = text_reg[31:24];
                        endcase
                    2'b01:
                         case((DrawX - 10'd336) >> 4)  // DrawX / 16
                            2'b00:
                                text_blk = extra_reg[7:0];
                             2'b01: 
                                text_blk = extra_reg[15:8];
                            2'b10:
                                text_blk = extra_reg[23:16];
                            2'b11: 
                                text_blk = extra_reg[31:24];
                         endcase
                endcase
                
                text_addr = (text_blk << 4) + text_inside_y;
                
                if(text_data[7 - text_inside_x])
                    begin   // drawing font black
                        Red   = 8'h00;
                        Green = 8'h00;
                        Blue  = 8'h00;
                    end
                // we are drawing the background(white)
                else
                   begin
                       Red   = 8'hff;
                       Green = 8'hff;
                       Blue  = 8'hff;
                   end
                
            end
        // draw text "time"
        else if ((DrawX >= 10'd544 && DrawX <= 10'd575) && (DrawY >= 10'd145 && DrawY <= 10'd160))
            begin
                text_inside_x = (DrawX - 10'd544) & 5'b00111;
                text_inside_y = (DrawY - 10'd145) & 5'b01111;
                
                // text_reg = "TIME"
                text_reg = 32'h454d4954;
 
                case((DrawX - 10'd544) >> 3)  // DrawX / 8
                2'b00:
                    text_blk = text_reg[7:0];
                2'b01: 
                    text_blk = text_reg[15:8];
                2'b10:
                    text_blk = text_reg[23:16];
                2'b11: 
                    text_blk = text_reg[31:24];
                endcase
                
                text_addr = (text_blk << 4) + text_inside_y;
                
                if(text_data[7 - text_inside_x])
                    begin   // drawing font black
                        Red   = 8'h00;
                        Green = 8'h00;
                        Blue  = 8'h00;
                    end
                // we are drawing the background(white)
                else
                   begin
                       Red   = 8'hff;
                       Green = 8'hff;
                       Blue  = 8'hff;
                   end
            end
            
        // draw timer 
        else  if ((DrawX >= 10'd540 && DrawX <= 10'd579) && (DrawY >= 10'd167 && DrawY <= 10'd182))
            begin
                text_inside_x = (DrawX - 10'd540) & 5'b00111;
                text_inside_y = (DrawY - 10'd166) & 5'b01111;
                
                if((DrawX >= 10'd540) && (DrawX <= 10'd547)) 
                  begin  // min10
                    text_blk = m10_index;  
                  end
                else if ((DrawX >= 10'd548) && (DrawX <= 10'd555))
                  begin  // min1
                    text_blk = m1_index;
                  end
                else if ((DrawX >= 10'd556) && (DrawX <= 10'd563))
                  begin // :
                    text_blk = 8'h3a; 
                   
                  end
                else if ((DrawX >= 10'd564) && (DrawX <= 10'd571))
                  begin // sec1
                    text_blk = s1_index;
                  end
                else if ((DrawX >= 10'd572) && (DrawX <= 10'd579))
                  begin // sec10
                    text_blk = s10_index;
                  end
                  
                // addr into font is test_blk * 16 + drawY % 16 // 
                text_addr = (text_blk << 4) + text_inside_y;
                
                if(text_data[7 - text_inside_x])
                    begin   // drawing font black
                        Red   = 8'h00;
                        Green = 8'h00;
                        Blue  = 8'h00;
                    end
                // we are drawing the background(white)
                else
                   begin
                       Red   = 8'hff;
                       Green = 8'hff;
                       Blue  = 8'hff;
                   end
            
            end
        // drawing instructions
        else if ((DrawX >= 10'd3 && DrawX <= 10'd122) && (DrawY >= 10'd82 && DrawY <= 10'd394))
            begin
            
            inside_x_wasd = (DrawX - 10'd3);
            inside_y_wasd = (DrawY - 10'd82);
            
            read_addr = inside_y_wasd * 120 + inside_x_wasd;
            Red = color_reg[read_data][23:16]; // want red in 110 * 195
            Green = color_reg[read_data][15:8];
            Blue = color_reg[read_data][7:0];
            
            end
        // we are drawing wins info 120 * 71
        else if ((DrawX >= 10'd515 && DrawX <= 10'd634) && (DrawY >= 10'd295 && DrawY <= 10'd365) && (wins != 3'b000))
            begin
            
            index_x_wins = (DrawX - 10'd515);
            index_y_wins = (DrawY - 10'd295);
            
            read_addr_wins = index_y_wins * 120 + index_x_wins;
            Red = color_reg[read_data_wins][23:16]; // want red in 120 * 71
            Green = color_reg[read_data_wins][15:8];
            Blue = color_reg[read_data_wins][7:0];
            
            end
        // we are drawing press info 384 * 20
        else if ((DrawX >= 10'd127 && DrawX <= 10'd510) && (DrawY >= 10'd446 && DrawY <= 10'd465) && (wins != 3'b000))
            begin
            
            index_x_press = (DrawX - 10'd127);
            index_y_press = (DrawY - 10'd446);
            
            read_addr_press = index_y_press * 384 + index_x_press;
            Red = color_reg[read_data_press][23:16]; // want red in 120 * 71
            Green = color_reg[read_data_press][15:8];
            Blue = color_reg[read_data_press][7:0];
            
            end
        // we are drawing outside of the board
        else
            begin
            Red   = 8'hff;
            Green = 8'hff;
            Blue  = 8'hff;
            end
   
    end 
  


endmodule
