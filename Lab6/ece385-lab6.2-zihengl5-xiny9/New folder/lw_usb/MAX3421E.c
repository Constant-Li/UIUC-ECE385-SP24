...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    BYTE buff[2] = {reg + 2, val};
    // select MAX3421E
    XSpi_SetSlaveSelect(&SpiInstance, 0x0001);

    ret = XSpi_Transfer(&SpiInstance, buff, NULL, 2);
    if (ret != XST_SUCCESS) {
        // deselect MAX3421E
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // write reg + 2 via SPI
    ret = XSpi_Transfer(&SpiInstance, s_buff, NULL, nbytes + 1);
    if (ret != XST_SUCCESS) {
        // deselect MAX3421E
        XSpi_SetSlaveSelect(&SpiInstance, 0x0000);
        xil_printf("Error! \n");
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    // write reg via SPI
    ret = XSpi_Transfer(&SpiInstance, s_buff, r_buff, 2);
    if (ret != XST_SUCCESS) {
        // deselect MAX3421E
        XSpi_SetSlaveSelect(&SpiInstance, 0x0000);
        xil_printf("Error! \n");
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // write reg via SPI
    ret = XSpi_Transfer(&SpiInstance, s_buff, r_buff, nbytes + 1);
    if (ret != XST_SUCCESS) {
        // deselect MAX3421E
        XSpi_SetSlaveSelect(&SpiInstance, 0x0000);
        xil_printf("Error! \n");
    ...
    // first idx in r_buff is garbage
